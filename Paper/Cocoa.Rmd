...
# Create external regressors matrix (exclude Price and Date)
xreg_all <- model %>% select(-Date, -Price)

# Fit ARIMAX (non-seasonal) model
arimax_model <- auto.arima(model$Price,
...
# External variables (must align in length with 'returns')
xreg_vars <- train %>%
  select(Temp, Fert, TFP_Index, Capital_Index, Land_Q, Labor_Q,
         Cropland_Q, prep, Production_tonnes, Yield_tonnes_per_hectare)

# Remove the first row to match diff (returns is 1 row shorter)
xreg_vars <- xreg_vars[-1, ]
...
# 2. ARIMAX (with external regressors)
xreg_test <- test %>% select(Temp, Fert, TFP_Index, Capital_Index, Land_Q,
                             Labor_Q, Cropland_Q, prep, Production_tonnes,
                             Yield_tonnes_per_hectare)

arimax_forecast <- forecast(arimax_model, xreg = as.matrix(xreg_test), h = nrow(test))
arimax_pred <- arimax_forecast$mean

# 3. GARCH
garch_forecast <- predict(garch_model, n.ahead = nrow(test))
garch_mean_return <- garch_forecast$meanForecast
last_price <- tail(train$Price, 1)
...
rmse_gam   <- rmse(gam_pred, actual)
rmse_arima <- rmse(arimax_pred, actual)
rmse_garch <- rmse(garch_pred, actual)

# ---- Output ----
cat("RMSE (GAM):   ", round(rmse_gam, 4), "\n")
cat("RMSE (ARIMAX):", round(rmse_arima, 4), "\n")
